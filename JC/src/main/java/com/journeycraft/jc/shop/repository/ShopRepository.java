...
public interface ShopRepository extends JpaRepository<Shop, Long> {
    ...
    Page<Shop> findByCuisine(String cuisine, Pageable pageable);
    ...
    @Query("SELECT s FROM Shop s WHERE (LOWER(s.name) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(s.address) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(s.description) LIKE LOWER(CONCAT('%', :keyword, '%'))) AND s.cuisine = :cuisine")
